  ...
  const { mutate: deletePost, isPending: isDeleting } = useMutation({
    mutationFn: async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            data.error || "Something went wrong while deleting the post",
          );
        }
        return data;
      } catch (error) {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["posts"] }); // Assuming the posts query has a queryKey of ["posts"]
    },
  });
  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    mutationFn: async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            data.error || "Something went wrong while liking the post",
          );
        }
        return data;
      } catch (error) {
    ...
    onSuccess: (updatedLikes) => {
      //invalidate the posts query to refetch the posts after liking
      // toast.success("Post liked successfully");
      // queryClient.invalidateQueries({ queryKey: ["posts"] });
      //this is not best practice to update the whole posts list after liking a post, ideally we should just update the liked post in the cache without refetching the whole list, but for simplicity we're invalidating the whole posts query here.
    ...
    onError: (error) => {
      toast.error(
        error.message || "Something went wrong while liking the post",
      );
    },
  });
